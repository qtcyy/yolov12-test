...
def parse_args():
    ...
    # 数据集参数
    parser.add_argument('--data', type=str, default='dota_dataset.yaml',
                        help='数据集配置文件路径')
    ...
    parser.add_argument('--project', type=str, default='runs/train',
                        help='保存项目路径')
    parser.add_argument('--name', type=str, default='dota_yolov12',
                        help='实验名称')
...
def check_and_fix_dataset(data_path):
    ...
    if not dataset_path.exists():
        ...
        print("   请先运行: python prepare_dota_dataset.py")
        return False
    ...
    try:
        ...
        config_dataset_path = Path(data_config['path'])
        
        # 如果配置文件中的路径不存在，更新为当前目录下的dataset
        if not config_dataset_path.exists():
            print(f"⚠️ 配置文件中的路径不存在: {config_dataset_path}")
            print(f"   更新为当前路径: {dataset_path}")
            
            data_config['path'] = str(dataset_path)
            
            # 保存更新后的配置
            with open(data_path, 'w', encoding='utf-8') as f:
                yaml.dump(data_config, f, default_flow_style=False, allow_unicode=True, indent=2)
            
            print(f"✓ 配置文件已更新")
            
    except FileNotFoundError:
        print(f"⚠️ 配置文件不存在，创建新的配置文件...")
    ...
    for split in ['train', 'val', 'test']:
        if split in data_config:
            ...
            label_exists = label_dir.exists() if split != 'test' else True  # test可以没有标签
            
            if img_exists:
                img_count = len(list(img_dir.glob('*.png')) + list(img_dir.glob('*.jpg')))
                print(f"  {split}集: {img_count} 张图像", end='')
                
                if label_exists and split != 'test':
                ...
                if split in ['train', 'val']:  # train和val是必需的
...
def train():
    ...
    if not check_and_fix_dataset(args.data):
        print("\n❌ 数据集检查失败，请先运行 prepare_dota_dataset.py")
        return
    ...
    # 测试推理
    print("\n测试推理...")
    test_img_dir = Path(args.data).parent / 'dataset' / 'images' / 'val'
